...
/**
 * @desc 重新 map 第三方服務資料
 * @param {host} string - 網址
 * @param {thirdParty} object - 第三方服務資料
 * @return {object} - 輸出帶有 url 的第三方服務資料
 */
export const mapThirdParty = (host, thirdParty) => {
  let env = '';
  switch (host) {
    case 'localhost:8080':
    case '88.8.194.112':
 ...
 * @param {url} string - 網址
 */
export const redirectThirdParty = ({ url, mixpanel = false }) => {
  const mrTrackingId = getCookie('mmo_tracking_id');
  const redirectUrl = mixpanel ? `${url}?tracking_id=${mrTrackingId}` : url;
